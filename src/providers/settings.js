...
class Settings {
    ...
    return this.model.findOne({ where: { client: this.client.user.id } }).then(instance => {
      if (instance) {
        try {
          ...
          this.client.emit('warn', `Unable to parse settings!`);
        }
        if (this.settings.suffix) { this.client.suffix = this.settings.suffix; }
      }
    });
  }
  ...
  set(key, value) {
    if (!this.settings[key] || this.settings[key] !== value) {
      ...
      this.model.upsert(
          { client: this.client.user.id, data: JSON.stringify(this.settings) },
          { where: { client: this.client.user.id } });
    }
