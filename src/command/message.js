...
class CommandMessage {
    ...
    run(){
        ...
        let reject = ( reason, response, error ) =>{
            ...
            let reject = this.command.onReject( this.channel, reason, error );
            if ( reject instanceof Promise ) {
                return reject.then( retVal => {
        ...
        try {
            ...
            const promise = this.command.run( this, this.args, this.executor, this.edited );
            if ( ( promise instanceof Promise ) ) {
                return promise.catch( err => { return reject( 'error', `An error occurred while running the command: \`${err.name}: ${err.message}\``, err ); } );
            }
        } catch(err) {
            return reject( 'error', `An error occurred while running '${this.command.name}' command: \`${err.name}: ${err.message}\``, err );
        }
    }
    ...
    getUser( search ) {
        ...
        if( !user ) {
            if ( !search ) {
                ...
                if ( !user ) {
                    user = this.client.users.find( user => user.username == search );
                    if ( !user && this.guild ) {
                        let member = this.guild.members.find( member => member.displayName == search  );
                        if ( member ) {
                            user = member.user;
                        }else{
                            user = undefined;
